...
function osintegration_outputxmlfeed() {
	...
	if( $options['localfimage'] ) {
		$fimages = array( '', '', '' );
		if( array_key_exists( 0, $recent_posts ) ) { $fimages[0] = '			<image id="1" src="' . osintegration_get_post_first_image( $recent_posts[0]['ID'], $recent_posts[0]['post_content'], $options ) . '"/>' . PHP_EOL; }
		if( array_key_exists( 1, $recent_posts ) ) { $fimages[1] = '			<image id="2" src="' . osintegration_get_post_first_image( $recent_posts[1]['ID'], $recent_posts[1]['post_content'], $options ) . '"/>' . PHP_EOL; }
		if( array_key_exists( 2, $recent_posts ) ) { $fimages[2] = '			<image id="3" src="' . osintegration_get_post_first_image( $recent_posts[2]['ID'], $recent_posts[2]['post_content'], $options ) . '"/>' . PHP_EOL; }
	}

	$titles = array( '', '', '' );
	if( array_key_exists( 0, $recent_posts ) ) { $titles[0] = '			<text id="1">' . $recent_posts[0]['post_title'] . '</text>' . PHP_EOL; }
	if( array_key_exists( 1, $recent_posts ) ) { $titles[1] = '			<text id="2">' . $recent_posts[1]['post_title'] . '</text>' . PHP_EOL; }
	if( array_key_exists( 2, $recent_posts ) ) { $titles[2] = '			<text id="3">' . $recent_posts[2]['post_title'] . '</text>' . PHP_EOL; }

	echo '<tile>' . PHP_EOL;
	echo '	<visual lang="en-US" version="2">' . PHP_EOL;
	echo '		<binding template="TileSquare150x150Text04" branding="logo" fallback="TileSquareImage">' . PHP_EOL;
	echo $titles[0];
	echo '		</binding>' . PHP_EOL;

	if( $options['localfimage'] ) {
		echo '		<binding template="TileWide310x150ImageAndText01" branding="logo" fallback="TileWideImage">' . PHP_EOL;
		echo $fimages[0];
		echo $titles[0];
	} else {
		echo '		<binding template="TileWide310x150Text05" branding="logo" fallback="TileWideText05">' . PHP_EOL;
		echo $titles[0];
		echo $titles[1];
		echo $titles[2];
	}
	...
	if( $options['localfimage'] ) {
		echo '		<binding template="TileSquare310x310SmallImagesAndTextList02" branding="logo">' . PHP_EOL;
		echo $fimages[0];
		echo $fimages[1];
		echo $fimages[2];
	} else {
		echo '		<binding template="TileSquare310x310TextList02" branding="logo">' . PHP_EOL;
	}
...
function osintegration_get_post_first_image( $postID, $post_content, $options ) {
	...
	if( has_post_thumbnail( $postID ) ) {
		...
		if( is_array( $attachments ) && !empty( $attachments ) ) {
			// The returned array is NOT zero based, instead the post_id of the attachment is used, so do a foreach loop through the array of 1.
