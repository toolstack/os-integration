...
class OSIntegration_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if( ( array_key_exists( 'enablefavicon', $options ) && $options['enablefavicon'] ) ||
			...
			if( osintegration_getoption( 'enablelivetile', $options )
				|| osintegration_getoption( 'enableios', $options )
				|| osintegration_getoption( 'enablewebapp', $options )
				|| osintegration_getoption( 'enablepwa', $options ) ) {
			...
			foreach( osintegration_store_list() as $store => $name ) {
				$ra_url = osintegration_getoption( $store . 'url', $options );

				if( $ra_url ) {
					$store_list .= '<a href="' . $ra_url . '"><img src="' . plugins_url( 'os-integration/images/' . $store . '.png' ) . '"></a>&nbsp;';
				}
			}
